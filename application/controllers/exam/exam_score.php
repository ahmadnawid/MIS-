 ...
 * */
 class Exam_score extends CI_Controller{
    public function __construct()
    {
        ...
        $this->load->model(array('get_public_data','exam_model','student_model'));
    ...
    public function index()
    {
        ...
        $data['subject'] = $this->exam_model->get_edu_subject(array('eduyear_semester_subjects.edu_year'=>1393));
        $this->load->view('exam/v_make_shuqa',$data);
        footer_tph();
    }
    //this function get corresponding semestet subject according to semester selected
    public function get_semester_subject()
    {
        $join_base = 'subject.subject_id=department_subject.subject_id';
        $where = array(
        'semester_id' => $this->input->post('semester'),
        'dep_id' => $this->input->post('dep')
        );
       $subject = $this->exam_model->get_dep_subject('department_subject',$join_base,$where);
       $html = '<option value="0">انتخاب نمایید</option>';
       foreach($subject as $sub)
       {
           $html.='<option value="'.$sub['subject_id'].'">'.$sub['sub_dari_name'].'('.$sub['sub_expresion'].')</option>';
       }  
       echo $html;
    }
     //this function get corresponding semestet subject according to semester selected
    ...
    {
        $where = array('eduyear_semester_subjects.edu_year'=>$this->input->post('edu_year'));
        $sem = '';
    ...
    public function make_exam_shuqa()
    {
		...
		$chance = $this->input->post('chance');
            ...
            $data['chance'] = $this->get_public_data->get_id('chance',array('chance_id'=>$chance),'chance_name');
                    $where = array('eduyear_semester_subjects.edu_year'=>$year);
                    if($this->input->post('semester'))
                    {
    ...
    {
        header_tph('');
        menu_tph('score_entry','exam');
        $data['department'] = $this->get_public_data->get_all_data('department');
        $data['semester'] = $this->get_public_data->get_all_data('semester');
        $data['year'] = $this->get_public_data->get_all_data('year','','year','desc');
        $data['chance'] = $this->get_public_data->get_all_data('chance');
        $this->load->view('exam/v_score_entry',$data);
        footer_tph(); 
    }
    ...
    {
        header_tph('');
        menu_tph('show_score','exam');
        $data['department'] = $this->get_public_data->get_all_data('department');
        $data['semester'] = $this->get_public_data->get_all_data('semester');
        $data['year'] = $this->get_public_data->get_all_data('year','','year','desc');
        $data['chance'] = $this->get_public_data->get_all_data('chance');
        $this->load->view('exam/v_search_score',$data);
        footer_tph(); 
    }
    public function searched_score()
    {
    ...
    {
      $counter = $this->exam_model->count_next_year('CEI90088',4);
      echo $counter;   
    }
 }