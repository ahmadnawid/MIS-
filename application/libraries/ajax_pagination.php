  ...
  class Ajax_pagination
  {
    ...
    function make_search($numrows,$starting=0,$per_page=10,$first_lb,$last_lb,$previous_lb,$next_lb,$page_process,$div_loading,$form_id='')
    {
        ...
        $this->perPage = '<select name="per_page" id="per_page" style="width:60px;margin-top:10px" onchange="load_page_pagination(\''.$page_process.'\',\''.$div_loading.'\',\'0\',\''.$form_id.'\')">
                                    <option value="10"';
                                    if($per_page==10){ $this->perPage .='selected="selected"';}
                                    $this->perPage .='>10</option><option value="20"';
                                    if($per_page==20){ $this->perPage .='selected="selected"';}
                                    $this->perPage .='>20</option><option value="50"';
                                    if($per_page==50){ $this->perPage .='selected="selected"'; }
                                    $this->perPage .='>50</option><option value="100"';
                                    if($per_page==100){ $this->perPage .='selected="selected"';}
                                    $this->perPage .='>100</option><option value="'.$numrows.'"';
